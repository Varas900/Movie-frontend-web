...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _loadSources() async {
    ...
    try {
      ...
      if ((widget.contentType ?? 'movie') == 'series' && widget.episodeId != null) {
      ...
      final watchNow = await _fetchWatchNow(widget.contentId);
  ...
  Future<void> _setupPlayerForSource(_PlayableSource s) async {
    try {
      ...
      final chewie = ChewieController(
        ...
        subtitle: _subtitlesEnabled ? Subtitles(_subtitleCues) : const Subtitles([]),
  ...
  Future<List<_PlayableSource>> _fetchWatchNow(int movieId) async {
    ...
    // Heuristic parsing: support single source or list in payload
    if (body is Map) {
      // Single-style
      final sourceUrl = body['sourceUrl'] ?? body['url'] ?? body['movieUrl'];
  ...
  Future<List<_PlayableSource>> _fetchEpisodeSources(int episodeId) async {
    final url = Uri.parse('${AppConstants.baseApiUrl}/movie/EpisodeSource/$episodeId');
    final res = await http.get(url);
    if (res.statusCode != 200) return [];
    final body = jsonDecode(res.body);
    final List<_PlayableSource> out = [];
  ...
  Future<List<_PlayableSource>> _attachSubtitles(List<_PlayableSource> sources) async {
    ...
    for (final s in sources) {
      ...
      try {
        final url = Uri.parse('${AppConstants.baseApiUrl}/api/MovieSubTitle/GetAllSubTitlesByMovieId/movie/GetAllSubTitlesBySourceID/${s.id}');
        final res = await http.get(url);
        if (res.statusCode == 200) {
          ...
          for (final sub in list) {
            if (sub is Map) {
              final lang = sub['language']?.toString() ?? sub['lang']?.toString() ?? 'Subtitle';
              final subUrl = sub['linkSubTitle']?.toString() ?? sub['subtitleUrl']?.toString() ?? sub['url']?.toString();
  ...
  Future<void> _maybeLoadSubtitlesForSource(_PlayableSource s) async {
    ...
    if (_selectedSubtitleUrl != null) {
      ...
      chosen = subs.firstWhere(
        (e) => (e['lang'] ?? '').toLowerCase() == _selectedSubtitleLang!.toLowerCase(),
        orElse: () => subs.first,
      );
    } else {
      chosen = subs.first;
  ...
  List<Subtitle> _parseSrtOrVttToChewie(String content) {
    ...
    while (i < lines.length) {
      ...
      final timeMatch = RegExp(r'(?<start>\d{2}:\d{2}:\d{2}[\.,]\d{3})\s*-->\s*(?<end>\d{2}:\d{2}:\d{2}[\.,]\d{3})').firstMatch(timeLine);
      if (timeMatch == null) { i++; continue; }
      ...
      if (start != null && end != null) {
        cues.add(Subtitle(index: cues.length, start: start, end: end, text: textLines.join('\n')));
      }
    }
    return cues;
  }
  ...
  Future<void> _reloadChewieForCurrentSource() async {
    ...
    final current = (_sources.isNotEmpty && _currentIndex < _sources.length) ? _sources[_currentIndex] : null;
  ...
  Future<void> _showSubtitleLanguagePicker(_PlayableSource s) async {
    if (s.subtitles.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No subtitles available')));
      return;
    }
        ...
        child: _isLoading
            ...
            : _error != null
                ...
                : _sources.isEmpty
                    ...
                    : ListView.separated(
                        itemCount: _sources.length > 1 ? _sources.length + 1 : 1,
                            ...
                            return Card(
                              child: AspectRatio(
                                ...
                                child: Stack(
                                  children: [
                                    _chewieController == null
                                        ? const Center(child: Text('Select a source to play'))
                                    ...
                                    Positioned(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          IconButton(
                                            tooltip: _subtitlesEnabled ? 'Subtitles: On (tap to turn off)' : 'Subtitles: Off (tap to turn on)'
                                                ,
                                            icon: Icon(_subtitlesEnabled ? Icons.closed_caption : Icons.closed_caption_disabled, color: Colors.white),
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () {
                                              final s = (_sources.isNotEmpty && _currentIndex < _sources.length)
                                                  ? _sources[_currentIndex]
                                                  : null;
                                              if (s != null) {
                                                _showSubtitleLanguagePicker(s);
                                              }
                                            },
                                          ),
                                        ],
                                    ...
                                    Wrap(
                                      ...
                                      children: s.subtitles.map((sub) {
                                        return OutlinedButton(
                                          onPressed: () => _openSource(_PlayableSource(url: sub['url']!, quality: sub['lang'])),
